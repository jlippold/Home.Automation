...
module.exports.forecast = function (callback) {
    ...
    async.auto({
        ...
        if (!results.outdoor.hasOwnProperty("query") || 
            !results.outdoor.query.hasOwnProperty("results") ||
            !results.indoor.hasOwnProperty("thermostatList") ||
            results.indoor.thermostatList.length !== 1) {

                return callback("Invalid json result");
        }
        //check for prop
        var outdoor = results.outdoor.query.results.channel;
        var indoor = results.indoor.thermostatList[0];

        var output = {
            date: moment().format("ddd MMM Do"),
            outdoor: {
                temperature: outdoor.item.condition.temp,
                text: outdoor.item.condition.text,
                high:  outdoor.item.forecast[0].high,
                low: outdoor.item.forecast[0].low,
                icon: getIcon(outdoor.item.condition.code),
                humidity: outdoor.atmosphere.humidity,
                wind: outdoor.wind.speed,
                sunrise: outdoor.astronomy.sunrise,
                sunset: outdoor.astronomy.sunset
            },
            indoor: {
                mode: indoor.settings.hvacMode,
        ...
        output.forecast = outdoor.item.forecast.map(function(item) {
            return {
                day: item.day,
                high: item.high,
                low: item.low,
...
function yahoo(callback) {
    var url = "https://query.yahooapis.com/v1/public/yql?q=" +
        "select%20*%20from%20weather.forecast%20where%20u=%27F%27%20AND%20woeid%20in%20(" +
        "select%20woeid%20from%20geo.places(1)%20where%20text=%2218302%22)&format=json";
    var options = {
        url: url,
        json: true
    };
